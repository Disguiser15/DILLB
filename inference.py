...
def inference(Trainer, cfg):
    ...
    for j,item in enumerate(dataset_dicts):
        ...
        match_quality_matrix = pairwise_iou(
            res[0]['instances'].pred_boxes.to(torch.device("cpu")), gt.pred_boxes
        )
        ...
        for i in range(len(res[0]['instances'])):
            box_info = {'confidence score':np.float(res[0]['instances'].scores.cpu().detach().numpy()[i]),
                        'iou with gt':np.float(iou_scores.cpu().detach().numpy()[i]),
                        'pred class':np.int(res[0]['instances'].pred_classes.cpu().detach().numpy()[i]),
                        'gt class':np.int(matched_classes.cpu().detach().numpy()[i]),
                        'pred box':res[0]['instances'].pred_boxes.tensor[i].cpu().detach().numpy().tolist(),
                        'gt box':matched_boxes[i].cpu().detach().numpy().tolist(),
...
if __name__ == '__main__':
    ...
    parser.add_argument("--static-file",type=str,default='output/try/try.json')
    parser.add_argument("--model-weights",type=str,default='/root/sjm/active-ssod-master/output/baseline/baseline/voc07train+voc12trainval_sup100/one_stage.pth')
    args = parser.parse_args()
    FILE_PATH = args.static_file #Json file of the intermediate process which you will use in tools/ScoreFuction.py
    # args.config_file = 'configs/voc/baseline/voc07train+voc12trainval_sup100_run1.yaml' #the config file you used to train this inference model
    args.eval_only = True
    args.resume = True
    args.num_gpus = 1
