...
from ubteacher.modeling.layer.gradient_scalar_layer import GradientScalarLayer
from ubteacher.modeling.layer.poolers import LevelMapper
...
class DomainAdaptationModule(torch.nn.Module):
    ...
    def __init__(self, cfg):
        ...
        num_ins_inputs = cfg.MODEL.ROI_BOX_HEAD.FC_DIM

        # self.USE_FPN = cfg.MODEL.RPN.USE_FPN
        # self.avgpool = nn.AvgPool2d(kernel_size=7, stride=7)

        self.consit_weight = 0.1

        self.grl_img = GradientScalarLayer(-1.0 * 0.01)
        self.grl_ins = GradientScalarLayer(-1.0 * 0.1)
        self.grl_img_consist = GradientScalarLayer(self.consit_weight * 0.01)
        self.grl_ins_consist = GradientScalarLayer(self.consit_weight * 0.1)
    ...
    def forward(self, proposals, img_features, da_ins_feature, da_ins_labels, da_proposals, targets=None, batched_inputs=None):
        """
        Arguments:
        ...
        da_img_loss, da_ins_loss, da_consistency_loss = self.loss_evaluator(
                ...
                da_ins_labels, targets, batched_inputs)
