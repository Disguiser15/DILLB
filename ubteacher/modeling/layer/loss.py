...
class DALossComputation(object):
    ...
    def __call__(self, proposals, da_img, da_ins, da_img_consist, da_ins_consist, da_ins_labels, targets, batched_inputs):
        """
        Arguments:
        ...
        for i, feat in enumerate(da_img):
            ...
            if 'is_source' in batched_inputs[0]:
                ...
                for i in range(len(batched_inputs)):
                    if batched_inputs[i]['is_source']==0:
                        mask_per_image = torch.zeros(1, dtype=torch.bool)
                    else:
                        mask_per_image = torch.ones(1, dtype=torch.bool)
                    masks.append(mask_per_image)
                da_img_label_per_level[masks, :] = 1
            # da_img_label_per_level = torch.zeros_like(feat, dtype=torch.float32)
            # da_img_label_per_level[masks, :] = 1
