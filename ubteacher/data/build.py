...
from ubteacher.data.common import (
    ...
    AspectRatioGroupedSemiSupDatasetTwoCropSource,
)
...
def divide_source_target(dataset_dicts):
    source_dicts = []
    target_dicts = []
    for i in range(len(dataset_dicts)):
        if dataset_dicts[i]['is_source'] == 1:
            source_dicts.append(dataset_dicts[i])
        else:
            target_dicts.append(dataset_dicts[i])
    return source_dicts, target_dicts
...
# uesed by supervised-only baseline trainer
def build_detection_semisup_source_train_loader(cfg, mapper=None):
    ...
    if sampler_name == "TrainingSampler":
        ...
        raise ValueError("Unknown training sampler: {}".format(sampler_name))
    return build_semisup_batch_data_loader_two_crop_source(
...
# batch data loader
def build_semisup_batch_data_loader_two_crop_source(
    dataset,
    sampler,
    total_batch_size_label,
    total_batch_size_unlabel,
