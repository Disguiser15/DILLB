...
def generate_xml():
    ...
    dataDir2 = args.input_dir2
    savepath = args.save_dir 
    img_dir = savepath+'JPEGImages/'
    anno_dir = savepath+'Annotations/'
    mkr(img_dir)
    mkr(anno_dir)
    with open(os.path.join(dataDir1,'ImageSets/Main/train.txt')) as f:
        train_07train_12trainval_original = set()
        for line in f.readlines():
            train_07train_12trainval_original.add(line.strip())
    with open(os.path.join(dataDir2,'ImageSets/Main/trainval.txt')) as f:
        for line in f.readlines():
            train_07train_12trainval_original.add(line.strip())

    random.seed(0)
    sample_num = math.floor(0.2*len(train_07train_12trainval_original))
    train_07train_12trainval = random.sample(train_07train_12trainval_original, sample_num)
    ...
    for xml_file in os.listdir(os.path.join(dataDir1,'Annotations/')):
        ...
        if root.find('object') is not None:
            if xml_file.split('.')[0] in train_07train_12trainval:
    ...
    for xml_file in os.listdir(os.path.join(dataDir2,'Annotations/')):
        if not xml_file.endswith('.xml'):
            continue

        annFile = os.path.join(dataDir2,'Annotations/')+xml_file
        print('--------parsing: '+annFile)
        tree = ET.parse(annFile)
        root = tree.getroot()
        if root.tag != 'annotation':
        ...
        if root.find('object') is not None:
            if xml_file.split('.')[0] in train_07train_12trainval:
                print('save {} to {}'.format(annFile,os.path.join(anno_dir,xml_file)))
                shutil.copy(os.path.join(dataDir2,'Annotations/'+xml_file.split('.')[0]+'.xml'), os.path.join(anno_dir,xml_file.split('.')[0]+'.xml'))
                shutil.copy(os.path.join(dataDir2,'JPEGImages/'+xml_file.split('.')[0]+'.jpg'), os.path.join(img_dir,xml_file.split('.')[0]+'.jpg'))
...
if __name__ == '__main__':
    import argparse
    parser = argparse.ArgumentParser(description='make 20% voc2007train+voc2012trainval dataset')
    parser.add_argument("--input-dir1",type=str,default='datasets/VOC2007/')
    parser.add_argument("--input-dir2",type=str,default='datasets/VOC2012/')
    parser.add_argument("--save-dir",type=str,default='datasets/index1/semi/voc_20%/')
    parser.add_argument("--dataseed",type=int,default=0)
